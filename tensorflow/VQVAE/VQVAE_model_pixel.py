...
class MODEL:
    def main(self, img):
        ...
        with tf.variable_scope('embed') :
            vq_dictionary = tf.Variable(tf.random.uniform([self.K, img_4.shape[3].value]), trainable=True, dtype=tf.float32, name='vq_dictionary')
            
            def calculate_embed(i):
                return tf.stack([vq_dictionary[tf.argmin(tf.reduce_mean(tf.pow( j - vq_dictionary, 2), axis=-1))] for j in tf.unstack(i, axis = 0)], axis = 0)
