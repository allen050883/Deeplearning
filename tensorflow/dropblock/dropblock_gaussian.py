...
def _bernoulli(shape, mean):
    return tf.nn.relu(tf.sign(mean - tf.random_uniform(shape, minval=0, maxval=1, dtype=tf.float32, seed=1234)))

def gaussian_noise(pic):
    g = np.random.normal(0, 255., pic)
    return tf.constant(g, dtype=tf.float32)
    

class DropBlock2D_gaussian(tf.keras.layers.Layer):
    def __init__(self, keep_prob, block_size, scale=True, **kwargs):
        super(DropBlock2D_gaussian, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.set_keep_prob()
        super(DropBlock2D_gaussian, self).build(input_shape)

    def call(self, inputs, training=None, **kwargs):
        self.inputs = inputs
        def drop():
            mask, mask_gaussian = self._create_mask(tf.shape(self.inputs))
            output = self.inputs * mask + mask_gaussian
#            output = tf.cond(self.scale,
#                             true_fn=lambda: output * tf.to_float(tf.size(mask)) / tf.reduce_sum(mask),
#                             false_fn=lambda: output)
            return tf.clip_by_value(output, 0, 255)

        if training is None:
            training = K.learning_phase()
        output = tf.cond(tf.logical_or(tf.logical_not(training), tf.equal(self.keep_prob, 1.0)),
                         true_fn=lambda: self.inputs,
                         false_fn=drop)
        return output
    ...
    def _create_mask(self, input_shape):
        sampling_mask_shape = tf.stack([input_shape[0],
                                       ...
                                       self.channel])
        mask_gaussian = gaussian_noise(self.inputs.shape); print(mask_gaussian)#(1, 126, 126, 1)
        
        mask = _bernoulli(sampling_mask_shape, self.gamma)
        mask = tf.pad(mask, self.padding)
        mask = tf.nn.max_pool(mask, [1, self.block_size, self.block_size, 1], [1, 1, 1, 1], 'SAME')
        print(mask)
        mask_gaussian = tf.multiply(mask, mask_gaussian)
        
        mask = 1 - mask
        
        return mask, mask_gaussian
